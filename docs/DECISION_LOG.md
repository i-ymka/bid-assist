...
## 2026-03-27 | [FIX] Retract loop + Freelancer retract API quirk (b972b3e)

**Суть:**
Когда `retract_bid()` падал с ошибкой, `price_line` оставался `None` → условие `if price_line and "retracted"` не срабатывало → цикл `schedule_bid_update` не прерывался → следующий scheduled check снова пытался ретрактнуть. Добавлен `break` при failed retract.

**Freelancer retract API quirk (задокументировано, не исправлено):**
Запрос `PUT /projects/0.1/bids/{id}/?action=retract` с телом `{}` возвращает `{"status": "success"}`, но бид остаётся активным (виден в профиле, кнопка Retract всё ещё есть). Воспроизводится на реальном аккаунте (yehia, bid 482133555). Причина неизвестна — возможно баг Freelancer, возможно проект был уже в закрытом состоянии. С нашей стороны запрос сформирован корректно.

**Также:** ошибка `400: You have used all of your bids` возникает при попытке ретрактнуть — странно, т.к. ретракт не тратит биды. Предположительно Freelancer использует один эндпоинт для bid actions и проверяет лимит даже для retract.

---

## 2026-03-27 | [UX] Shared round-robin цвета проектов (ca439c0 → 452b07c)

**Суть:**
Два итерации:

1. `ca439c0` — заменили `project_id % 5` на round-robin с памятью в процессе. Переупорядочена палитра: `plum1 → aquamarine1 → gold1 → light_slate_blue → yellow3`.

2. `452b07c` — перенесли счётчик в `shared_analysis.db` (таблица `project_colors`). Оба бота атомарно назначают следующий цвет при первом упоминании project_id через `INSERT OR IGNORE ... (SELECT COUNT(*) % 5 FROM project_colors)`. Локальный `_color_cache` избегает повторных обращений к БД.

**Мотивация:**
- `project_id % 5` → фактически случайный результат для больших ID Freelancer
- In-process counter (ca439c0) → каждый бот стартует с 0, в объединённом логе два подряд розовых
- Shared DB → единый счётчик для обоих процессов, гарантированный round-robin в общем потоке

---

## 2026-03-27 | [TEMP] Call 1 переключён на flash-3 (pro-3.1 постоянно overloaded)

**Суть:**
`GEMINI_MODEL=gemini-3-flash-preview` в обоих `.env` на сервере (было `gemini-3.1-pro-preview`). Изменение только на сервере, локальные `.env` не трогались. Код не менялся — только конфиг.

**ВРЕМЕННО.** Вернуть `gemini-3.1-pro-preview` когда Google решит проблему с overload на стороне сервера.

**Мотивация:**
Анализ логов за 30 часов: pro-3.1 ответил успешно **20 раз** vs **332 overload**. Успешность ~6% — бот фактически всегда работал на flash-3 через fallback, просто с задержкой 4-10 мин на попытку pro. Убрали холостые вызовы.

**Реальность:**
- При overload flash-3 (маловероятно): код делает 3 ретрая без задержки (CLI сам ~2 мин на попытку)
- Качество Call 1 может снизиться (flash умнее по бенчмаркам чем 2.5-pro, но хуже чем 3.1-pro в рассуждениях)
- Зато latency Call 1: с ~10 мин → ~2-3 мин
